...
class DailyStockTechnicalAnalysisService:
    ...
    async def run_once(self, analysis_trade_date: str | None = None) -> DailyStockTechRunStats:
        # analysis_trade_date：盘前分析结果所属交易日（用于读股票池 + 结果表 trade_date）
        resolved_analysis_trade_date = self._normalize_trade_date(
            analysis_trade_date or self.resolve_target_trade_date()
        )
        stats = DailyStockTechRunStats(target_trade_date=resolved_analysis_trade_date)

        market_analysis_doc = await self._load_market_analysis_doc(resolved_analysis_trade_date)
        if not market_analysis_doc:
            logger.info(
                "market analysis doc not found, analysis_trade_date=%s",
                resolved_analysis_trade_date,
            )
            return stats

        # 以盘前分析文档内的 trade_date 作为最终语义来源
        analysis_trade_date = self._normalize_trade_date(
            market_analysis_doc.get("trade_date") or resolved_analysis_trade_date
        )
        kline_end_trade_date = self._normalize_trade_date(
            market_analysis_doc.get("prev_trade_date") or ""
        ) or analysis_trade_date

        stock_pool = self._extract_stock_pool(market_analysis_doc)
        stats.target_trade_date = analysis_trade_date
        stats.total = len(stock_pool)

        if not stock_pool:
            logger.info("stock pool is empty, analysis_trade_date=%s", analysis_trade_date)
            return stats
        ...
        if len(succeeded_codes) == len(stock_pool):
            logger.info(
                "all stocks already succeeded, skip whole batch, analysis_trade_date=%s, total=%s",
                analysis_trade_date,
                len(stock_pool),
            )
            return stats
        ...
        logger.info(
            "daily stock technical analysis start, analysis_trade_date=%s, kline_end_trade_date=%s, total=%s, pre_skipped_succeeded=%s, to_process=%s, worker_concurrency=%s",
            analysis_trade_date,
            kline_end_trade_date,
            stats.total,
        ...
        async def _run_one(stock_item: dict[str, Any]) -> None:
            async with semaphore:
                await self._process_one_stock(
                    ...
                    kline_end_trade_date=kline_end_trade_date,
        ...
        logger.info(
            "daily stock technical analysis finished, analysis_trade_date=%s, kline_end_trade_date=%s, total=%s, pre_skipped_succeeded=%s, to_process=%s, claimed=%s, succeeded=%s, failed=%s, skipped_data_insufficient=%s, skipped_by_other_worker=%s",
            analysis_trade_date,
            kline_end_trade_date,
            stats.total,
            stats.pre_skipped_succeeded,
    ...
    async def _load_market_analysis_doc(self, analysis_trade_date: str) -> dict[str, Any] | None:
        raw_date = analysis_trade_date.replace("-", "")
        return await self.daily_market_analysis_repository.get_by_trade_date(raw_date)

    @staticmethod
    def _extract_stock_pool(market_analysis_doc: dict[str, Any]) -> list[dict[str, Any]]:
        result: list[dict[str, Any]] = []
        sector_top_stocks = market_analysis_doc.get("sector_top_stocks") or []
        ...
        *,
        ...
        try:
            raw_bars = await self.daily_kline_snapshot_repository.get_recent_bars(
                ...
                end_trade_date=kline_end_trade_date,
                limit=30,
            )
