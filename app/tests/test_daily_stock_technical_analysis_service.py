...
class DailyStockTechnicalAnalysisServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _build_service(self, result_repo, bars_by_symbol, resolver=None):
        market_repo = FakeDailyMarketAnalysisRepository({"20260110": build_stock_pool_doc()})
        kline_repo = FakeDailyKLineSnapshotRepository(bars_by_symbol=bars_by_symbol)
        return DailyStockTechnicalAnalysisService(
    ...
    async def test_kline_insufficient_then_skipped(self):
        repo = FakeTechnicalResultRepository()
        service = self._build_service(repo, {"000001": build_bars(10), "000002": build_bars(30)})
    ...
    async def test_non_trade_day_fallback_to_previous_trade_date(self):
        repo = FakeTechnicalResultRepository()
        market_repo = FakeDailyMarketAnalysisRepository({"20260109": build_stock_pool_doc()})
        kline_repo = FakeDailyKLineSnapshotRepository({"000001": build_bars(30), "000002": build_bars(30)})
        service = DailyStockTechnicalAnalysisService(
            daily_market_analysis_repository=market_repo,
            daily_kline_snapshot_repository=kline_repo,
            technical_result_repository=repo,
            resolve_target_trade_date=lambda: "2026-01-09",
            worker_concurrency=4,
            running_timeout_minutes=10,
        )
